...
def days_difference_calculator ():
    start_date = get_valid_date('Enter the start date in the ‘YYYY-MM-DD’ format: ')
    end_date = get_valid_date('Enter the end date in ‘YYYY-MM-DD’ format: ')
    days_difference = abs((end_date - start_date).days)
    print ('The days difference is ' + str(days_difference) + ' day(s).')
